...
const MyResult = ({API_BASE = 'https://quiz-app-fsci.onrender.com'}) => {
...
// Effect: Fetch results when component mounts or when selectedTechnology changes
  ...
  }, [API_BASE, selectedTechnology, getAuthHeader]);


// Effect: fetch all results once (or when API_BASE changes) to build a list
...
// It aggregates totals and computes an overall percentage.
  ...
  return (
    ...
    <div className={resultStyles.filterButtons}>
     ...
     <button onClick={() => handleSelectTech("all")} className={`${resultStyles.filterButtonActive} 
    ...
    {/* if we don't yet have technologies but result exist, derive from results*/}
    ...
    return (
      <article className={resultStyles.card}>
        ...
        <div className={resultStyles.cardText}>
          <h3 className={resultStyles.cardTitle}>{item.title}
            </h3> 
        <div className={resultStyles.cardMeta}>{item.totalQuestions}Qs{item.timeSpent ? `${item.timeSpent}` : ""}
        </div>    
        </div>
        </div>
